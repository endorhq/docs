---
title: Project Configuration (rover.json)
...
The `rover.json` file is Rover's project configuration file that controls how AI agents interact with your codebase. This reference documentation explains each configuration field, provides practical examples, and covers security best practices.

## Introduction

The `rover.json` file:

- Lives in your project root directory
- Is committed to version control and shared across your team
- Configures AI agent behavior, environment detection, and runtime settings
- Is automatically created when you run `rover init`

**Key distinction:** `rover.json` defines project-wide settings, while `.rover/settings.json` stores user-specific preferences (like user preferences). Only commit `rover.json` to version control.
...
Rover uses schema versioning for automatic migration of configuration files. When you open a project with an older schema, Rover automatically migrates it while preserving your settings.

### Configuration vs Settings

| Type | File | Purpose | Commit? |
|------|------|---------|---------|
| **Project Config** | `rover.json` | Shared project settings | ✅ Yes |
| **User Settings** | `.rover/settings.json` | Personal preferences, API keys | ❌ No |
...
- **[Schema Versioning](docs/schemas.md)**: Learn about Rover's schema migration approach
- **[Rover Init Command](README.md)**: How `rover init` creates `rover.json`
- **[Model Context Protocol](https://modelcontextprotocol.io/)**: Available MCP servers and guides
- **[User Settings](.rover/settings.json)**: Configure personal preferences

---

**Questions or issues?** Verify your `rover.json` is valid JSON with correct field values. Rover auto-migrates schema versions on load—you don't need to manage versions manually.
