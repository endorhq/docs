...
When you create a task, Rover copies the selected workflow into the [task folder](/rover/concepts/task/#local-structure). Then, the Rover agent manager that runs in the [sandboxed environment](/rover/concepts/sandbox) reads the workflow file and executes each step sequentially.
...
Custom workflows are required to follow the [Workflow YAML file format documented here](../../reference/workflow-file).

If you want to start from an existing workflow, you can first dump its contents to a file in the filesystem:

```sh
rover workflows inspect swe --json | jq .workflow > improved-swe.json
```

Make the changes you want on the `improved-swe.json`, and import it.

```sh
rover workflows add --name improved-swe improved-swe.json
```
